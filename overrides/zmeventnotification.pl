...
my $fcm_v1_key;
my $fcm_v1_url;
my $fcm_service_account_file;
my $cached_access_token;
my $cached_access_token_expiry = 0;
...
sub loadEsConfigSettings {
  ...
  $fcm_service_account_file=
    config_get_val( $config, 'fcm', 'fcm_service_account_file' );
...
sub get_google_access_token {
  ...
  if (time() < $cached_access_token_expiry) {
      return $cached_access_token;
  }
  
  if ( !try_use('Crypt::OpenSSL::RSA') ) {
    printError("Crypt::OpenSSL::RSA is required for Service Account Auth");
    return undef;
  }
  
  local $/;
  open( my $fh, '<', $key_file ) or do {
    printError("Could not open service account file $key_file: $!");
    return undef;
  };
  my $json_text = <$fh>;
  close($fh);
  ...
  my $encoded_header = encode_base64(encode_json($header), '');
  $encoded_header =~ s/\+/\-/g; $encoded_header =~ s/\//_/g; $encoded_header =~ s/=+$//;
  $encoded_header =~ s/\n//g;
  
  my $encoded_claim = encode_base64(encode_json($claim), '');
  $encoded_claim =~ s/\+/\-/g; $encoded_claim =~ s/\//_/g; $encoded_claim =~ s/=+$//;
  $encoded_claim =~ s/\n//g;
  ...
  $rsa->use_sha256_hash();
  my $signature = $rsa->sign($payload);
  
  my $encoded_signature = encode_base64($signature, '');
  $encoded_signature =~ s/\+/\-/g; $encoded_signature =~ s/\//_/g; $encoded_signature =~ s/=+$//;
  $encoded_signature =~ s/\n//g;
  ...
  if ($res->is_success) {
    ...
    $cached_access_token_expiry = time() + $token_data->{expires_in} - 60;
    return $cached_access_token;
  } else {
    printError("Failed to get access token: " . $res->status_line . " " . $res->decoded_content);
    return undef;
  }
}
